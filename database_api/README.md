...
- The shared `db` object is a `flask_sqlalchemy.SQLAlchemy` instance; models subclass `db.Model`, and both `frontend_multiuser` (admin UI) and `worker_plan_database` expect to initialize it via `db.init_app(app)`.
