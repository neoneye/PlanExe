Pyright setup for `frontend_gradio`
===================================

TL;DR
-----
- Scope: This config is local to `frontend_gradio` and only affects editor analysis, not runtime.
...
--------------------
- Cursor/Pyright could not resolve imports like `worker_plan_api.*` when editing `frontend_gradio/app.py`.
- The code lives in `frontend_gradio/`, but `worker_plan_api` lives next door in `../worker_plan`, so the editor needed a path hint.
