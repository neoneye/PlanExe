...
class DocumentDetails(BaseModel):
    ...
    task_for_editor: str = Field(
        description="A task for the editor to improve the document"
    )


MY_SYSTEM_PROMPT = """
Your job is to identify a range of lines where 'the-document.txt' has to be modified, in order to improve the document.

Over multiple refinements, the goal is that the document satisfies the SMART criteria.

In the 'rationale' field, explain why it has to be changed. What is your impression of the old text, was it incorrect, incomplete, misleading, etc?

The 'range_start' and 'range_end' fields are the line numbers where the change starts and ends.
"""
...
5;placeholder, I'm an empty line
