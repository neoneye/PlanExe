...
PROMPT> python -u -m planexe.viability.measure4 | tee output.txt
"""
import json
import logging
...
You must answer the checklist items with the following ids:
{json_expected_ids}

You job is to answer the checklist items that has status TODO.
...
# Output Format
- Your response MUST be a single, valid JSON object matching the `ChecklistResponse` schema.
- The `checklist_answers` list MUST contain an entry for EVERY checklist item with status TODO. It must NEVER be empty or incomplete.
