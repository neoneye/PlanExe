...
ALL_CHECKLIST_ITEMS = [
    {
        ...
        "instruction": "Assess whether the project's core premise, goal, or claimed mechanisms contradict well-established scientific laws or consensus (e.g., conservation of energy, second law of thermodynamics, causality and the speed limit from relativity, quantum mechanics, germ theory, evolution). Do NOT limit your check to implementation methods; judge the subject matter itself. Set LEVEL=HIGH if any success condition requires a physical impossibility or treats pseudoscience as fact, or if overturning a well-tested law is assumed without extraordinary, independent, peer-reviewed evidence. Otherwise set LEVEL=LOW. In the justification: (1) name the specific principle(s) implicated; (2) quote the exact line(s) from the plan that create the conflict; (3) in one sentence, explain why this makes the plan non-viable. Mitigation for HIGH must be a stop/go fatal-flaw gate: remove/replace the invalid premise OR proceed only if independent, peer-reviewed, replicated evidence resolves the conflict. Do not propose mitigations that merely document, rebrand, or defer the same invalid claim. Examples of HIGH triggers (non-exhaustive): perpetual-motion/over-unity devices, reactionless drives, faster-than-light effects, anti-gravity shielding, homeopathy beyond Avogadro’s limit, astrology presented as predictive science.",
