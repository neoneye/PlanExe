...
ALL_CHECKLIST_ITEMS = [
    ...
    },
    {
        ...
        "instruction": "Assess the project timeline for realism, consistency, and resource backing. Your assessment must cover four areas:\n1. Internal Consistency: Check for contradictions. Do phase durations, milestones, and the overall completion date align? Are dependencies logical, reflecting necessary lead times (e.g., procurement, approvals)?\n2. Resource Realism: Verify that a single, resource-loaded baseline schedule exists, detailing the work breakdown, task ownership, and a clear critical path.\n3. Benchmarking: Compare the plan's durations against analogous projects or industry norms for activities of similar scope and complexity.\n4. Uncertainty Quantification: The plan must quantify schedule risk (e.g., P10/P50/P90 estimates). If unavailable, state that uncertainty is unquantified.\n\nSet LEVEL to HIGH if:\n- Internal contradictions exist (e.g., dates do not add up).\n- No resource-loaded baseline schedule with a critical path is provided.\n- The timeline relies on unconfirmed external approvals or lead times that are not under the project's control.\n\nIn the justification, cite the specific contradictions, missing artifacts, or benchmark gaps. In the mitigation, require a resource-loaded master schedule (with dependencies, critical path, and buffers) and a schedule-risk summary (P10/P50/P90) within 60 days. Include a NO-GO gate if the reconciled P50 date misses critical external deadlines.",
    ...
    },
    {
        ...
        "instruction": "Assess the end-to-end financial viability of the plan. Your assessment must cover four areas:\n1. Cost Structure: The plan must provide a Cost Breakdown Structure (CBS) aligned to the Work Breakdown Structure (WBS), including CAPEX/OPEX, contingencies, and all associated costs (insurance, permits, financing).\n2. Funding & Revenue: Identify all funding sources and classify them as 'committed' vs. 'indicative'. If a revenue model exists, distinguish proven mechanisms from speculative ones.\n3. Economic Model: The plan must include a cash-flow model. For revenue-generating projects, this must include NPV/IRR/payback calculations.\n4. Risk Analysis: The financial model must be linked to schedule risk and include sensitivity analysis or probabilistic forecasts (e.g., P10/P50/P90 cost estimates).\n\nSet LEVEL to HIGH if:\n- No credible CBS exists.\n- Primary funding sources are uncommitted or highly contingent.\n- The model relies on unproven monetization without a clear de-risking plan.\n- The reconciled P50 cost estimate exceeds the available budget by more than 25%.\n\nIn the justification, cite the specific missing artifacts or quantified gaps. In the mitigation, require an independent bottom-up cost estimate and cash-flow model within 60 days. Include a NO-GO gate if the P50 cost exceeds the funding ceiling by >25%.",
