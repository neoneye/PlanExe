...
- **3–5** prioritized blockers (not 20+ vague risks)
...
• **Schema (Pydantic)** — stable, typed contracts between LLM and engine:
  ...
  - `ViabilityBlockerCandidate` → `ViabilityBlocker` (with `quick_win_step_ids`)
  ...
  - **Metadata (recommended):** `prompt_id`, `config_version`,
    `rom_dependencies` (e.g., ["FinancialModel v2.1","RiskRegister v4"])
...
• **System prompt (`VIABILITY_SYSTEM_PROMPT`)** — JSON-only instructions for the LLM
  to extract blockers and synthesize steps with strict acceptance tests, artifacts,
  quick-win bias, and GRAY handling for unknowns.
  **Reality:** LLM output quality is a major dependency; keep the prompt versioned,
  regression-tested on a gold set, and evolve it deliberately.
...
- **Blocker score (selection):**
  ...
  *Micro-example (e.g., ethics capture risk):* 0.9 × 0.8 × 0.7 × (1−0.4) = **0.3024**.
  (Introduce domain weights/normalization via `EngineConfig` if needed.)

- **ROM (Return on Mitigation):**
      (ΔP_success × Project_EV + Σ risk_reduction_i × impact_i) / (effort_days + direct_cost)
  Used to sort blockers **after** actionability checks.
  **Input dependency:** ROM quality depends on upstream EV/risk models; garbage in → garbage out.

- **Anti-greenwashing:** GREEN requires artifacts; partial evidence remains **YELLOW**
  until required proofs are attached. Cap color upgrades per run to avoid cosmetic flips.
...
- **<3 actionable blockers:** include the best GRAY-resolver tasks as provisional steps to
  reach actionable state; keep total ≤5.
- **Conflicts/trade-offs:** flag in `ViabilitySummary` with a one-liner (e.g., “Budget ↑ vs. timeline ↑”);
  prefer higher ROM unless domain rules override.
- **User overrides:** allow callers to pin/include a blocker; still apply evidence gating.

Governance & versioning (operational discipline)
------------------------------------------------
- Treat `EngineConfig` and the prompt as **versioned, review-gated assets**:
  - Bump `config_version` on parameter changes; require a calibration note in PRs.
...
Intended flow
-------------
1) Upstream LLM (using `VIABILITY_SYSTEM_PROMPT`) emits ≤15 `ViabilityBlockerCandidate`s with draft `Step`s.
...
Design promises
---------------
- **Focus over frenzy:** never more than five blockers.
...
- “Pinned” blocker respected even if low ROM; trade-off note emitted.

PROMPT> python -m planexe.plan.viability_assessor6
"""
